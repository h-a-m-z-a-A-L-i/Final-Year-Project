#!/usr/bin/env python3
"""Live smoke test for creating_markdown_by_index."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Smoke test creating_markdown_by_index tool")
    parser.add_argument("--url", default=DEFAULT_URL, help="Open notebook /edit URL")
    parser.add_argument("--index", type=int, default=1, help="1-based anchor cell label")
    ...
    payload = {"url": args.url, "index": args.index}
    if args.tab_id is not None:
        payload["tab_id"] = args.tab_id

    print("Calling creating_markdown_by_index with:", json.dumps(payload, indent=2))
    result = registry().call("creating_markdown_by_index", payload)
    print("\nResult:")
