...
def test_inject_prefetched_tool_context_uses_system_not_tool_role():
    ...
    assert "notebook_get_cell" in system
    assert json.dumps({"ok": True, "cells": []}) in system


def test_inject_prefetched_tool_context_tail_keeps_system_stable():
    messages = [{"role": "system", "content": "base"}, {"role": "user", "content": "hi"}]
    inject_prefetched_tool_context(
        messages,
        graph={"ok": True, "cells": []},
    ...
    assert "Prefetched tool results" in messages[1]["content"]
    assert messages[1]["content"].endswith("hi") or "hi" in messages[1]["content"]


def test_inject_prefetched_tool_context_inserts_system_when_missing():
    messages = [{"role": "user", "content": "hi"}]
    inject_prefetched_tool_context(messages, graph={"nodes": 1}, target="system")
