...
def test_capture_edit_baseline():
    ...
    with patch("testing.host.notebook_context.load_notebook_snapshot", return_value=(snap, "live")):
...
def test_wait_for_edit_verification_snapshot():
    before = {"cells": [{"index": 2, "type": "code", "input": "old"}]}
    after = {"cells": [{"index": 2, "type": "code", "input": "print(42)"}]}
    calls = [0]

    def _load(_url):
        calls[0] += 1
        return (after if calls[0] > 1 else before, "live")

    with patch("testing.host.notebook_context.load_notebook_snapshot", side_effect=_load):
    ...
    assert out["wait_reason"] == "snapshot_input"
