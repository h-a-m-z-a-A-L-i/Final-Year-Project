...
def _prune_rate_tracker(tracker):
...
def _record_request_attempt(attempt_id: str):
    ...
    tracker = _prune_rate_tracker(tracker)
...
def _check_rate_limits():
    ...
    for event in events:
        ...
        req_count = int(event.get("requests", 1) or 1)
...
def _wait_for_request_slot():
    ...
    rpm = sum(1 for event in events if datetime.fromisoformat(event["timestamp"]) >= one_minute_ago)
    rph = sum(1 for event in events if datetime.fromisoformat(event["timestamp"]) >= one_hour_ago)
    rpd = sum(1 for event in events if datetime.fromisoformat(event["timestamp"]) >= one_day_ago)
    ...
    waits = []
    if rpm >= RPM_LIMIT:
        oldest = min(datetime.fromisoformat(e["timestamp"]) for e in events if datetime.fromisoformat(e["timestamp"]) >= one_minute_ago)
        waits.append((oldest + timedelta(minutes=1) - now).total_seconds())
    if rph >= RPH_LIMIT:
        oldest = min(datetime.fromisoformat(e["timestamp"]) for e in events if datetime.fromisoformat(e["timestamp"]) >= one_hour_ago)
        waits.append((oldest + timedelta(hours=1) - now).total_seconds())
    if rpd >= RPD_LIMIT:
        oldest = min(datetime.fromisoformat(e["timestamp"]) for e in events if datetime.fromisoformat(e["timestamp"]) >= one_day_ago)
        waits.append((oldest + timedelta(hours=24) - now).total_seconds())

    wait_seconds = max(0.1, max(waits) if waits else 0.1)
    print(f"[RATE LIMIT] Waiting {wait_seconds:.1f}s for request slot...")
    time.sleep(wait_seconds)


def _ask_once(conv_id: str, prompt_text: str):
    if client is None:
        print("[ERROR] Missing CEREBRAS_API_KEY environment variable.")
        return
    ...
    while True:
        try:
            ...
            if "429" in err or "queue_exceeded" in err or "too_many_requests_error" in err:
                print(f"Assistant: Queue busy. Retrying in 2.5s...")
                time.sleep(2.5)
                continue
            else:
    ...
    t_toks = len(prompt_text.split()) + len(full_text.split())
    _finalize_request_attempt(attempt_id, t_toks)
...
def main():
    ...
    if client is None:
        print("[ERROR] Set CEREBRAS_API_KEY before running this script.")
        return
