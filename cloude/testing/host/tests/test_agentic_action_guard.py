...
def test_agentic_prompt_contains_mandatory_two_phase():
    path = Path(__file__).resolve().parents[1] / "prompts" / "agentic.txt"
    text = path.read_text(encoding="utf-8")
    assert "MANDATORY two-phase" in text
    assert "Split cell 38 into 3 smaller cells" in text
    assert "fix the error in cell 39" in text.lower() or "Fix cell 39" in text or "cell 39" in text
    assert "notebook_get_cell" in text
    assert "remove cells 2, 4, 7, 3, 6, 5" in text.lower()
    assert "descending" in text.lower()
    assert "host sorts" in text.lower() or "host executes" in text.lower()
