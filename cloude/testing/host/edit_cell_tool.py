"""edit_cell_by_index tool — dispatch content set (fire-and-forget)."""
...
try:
    ...
    from .edit_cell_verification import EDIT_DISPATCH_TIMEOUT_SEC, dispatch_edit_cell
except Exception:
    from bot_tool_utils import (
        BROWSER_EDIT_MAX_WAIT_MS,
        BROWSER_EDIT_TIMEOUT_SEC,
...
def run_edit_cell(args: dict) -> dict:
    ...
    dispatch_timeout = float(args.get("dispatch_timeout") or EDIT_DISPATCH_TIMEOUT_SEC)
    started = time.monotonic()

    attempt_cmd = dict(cmd)
    attempt_cmd["maxWaitMs"] = BROWSER_EDIT_MAX_WAIT_MS
    attempt_cmd["timeout"] = BROWSER_EDIT_TIMEOUT_SEC
    dispatch = dispatch_edit_cell(attempt_cmd, timeout=dispatch_timeout)
    elapsed_ms = round((time.monotonic() - started) * 1000.0, 1)

    if not dispatch.get("ok"):
        error = str(dispatch.get("error") or "edit dispatch failed")
