...
def extract_target_cell_index_from_prompt(prompt: str) -> int | None:
    """Parse target cell index from split/fix/edit requests."""
    try:
        from .agentic_tool_chain import extract_target_cell_index
    except Exception:
        from agentic_tool_chain import extract_target_cell_index
    return extract_target_cell_index(prompt)


def is_phase0_query_tool(name: str) -> bool:
    return str(name or "").strip() in PHASE0_QUERY_TOOLS


def is_phase0_query_only_batch(parsed_tools: list[str] | None) -> bool:
    names = [str(t).strip() for t in (parsed_tools or []) if str(t).strip()]
    if not names:
        return False
    return all(n in PHASE0_QUERY_TOOLS for n in names)


def batch_has_write_tools(parsed_tools: list[str] | None) -> bool:
    return not batch_lacks_write_tools(parsed_tools)


def build_phase0_query_nudge(prompt: str) -> str:
    target = extract_target_cell_index_from_prompt(prompt)
    if target is not None:
        hint = f"Call notebook_get_cell({target}) to read the source."
    else:
        hint = (
            "Call notebook_get_cell(N) for the target cell, or notebook_list_cells "
            "if indices are unknown."
        )
    return (
        "Phase 1 (query only): read the notebook — do NOT insert, edit, delete, or run yet. "
        f"{hint} notebook_snapshot_status is allowed. No write tools on this call.\n"
        f"Task: {prompt.strip()[:300]}"
    )


def build_phase1_implementation_nudge(prompt: str) -> str:
    split_hint = ""
    if prompt_requests_split_cell(prompt):
        split_hint = (
            " For split: insert empty cells below the source index, then edit with portions of "
            "the ORIGINAL source from phase 1 — never shrink the source cell only."
        )
    run_hint = ""
    if user_requests_run(prompt):
        run_hint = " Include run_cell after edit if the user asked to run."
    return (
        "Phase 2 (implement): use the source from phase 1. Dispatch insert_cell, "
        "edit_cell_by_index, delete_by_index, and run_cell as needed — no notebook_list_cells."
        f"{split_hint}{run_hint}\n"
        f"Task: {prompt.strip()[:300]}"
    )


def build_auto_injected_query_tool_call(
    prompt: str,
    *,
    url: str,
    tab_id: int | None = None,
) -> dict:
    """Native tool_call dict for host-injected round-0 reads."""
    import json

    target = extract_target_cell_index_from_prompt(prompt)
    if target is not None:
        args: dict = {"url": url, "cell_index": int(target)}
...
def split_missing_inserts(prompt: str, tool_calls: list | None) -> bool:
    """Round-1 split batch that edits the source cell without insert_cell."""
    if not prompt_requests_split_cell(prompt):
        return False
    target = _split_source_cell_index(prompt)
    if target is None:
    ...
    for call in tool_calls or []:
        ...
        if name == "insert_cell":
            has_insert = True
        if name == "edit_cell_by_index":
            args = getattr(call, "args", None) or (
                call.get("args") if isinstance(call, dict) else {}
            ) or {}
            try:
                if int(args.get("cell_index")) == int(target):
                    edit_only_source = True
            except (TypeError, ValueError):
                pass
    return has_write and edit_only_source and not has_insert


def build_split_requires_inserts_nudge(prompt: str) -> str:
    target = _split_source_cell_index(prompt)
    cell_hint = f"cell {target}" if target is not None else "the source cell"
    return (
        f"Split requires insert_cell (empty) below {cell_hint}, then edit multiple cells with "
        "portions of the original source. Do NOT only edit/shrink the source cell.\n"
        f"Task: {prompt.strip()[:300]}"
    )


def build_round0_writes_blocked_nudge(prompt: str) -> str:
    return (
        "Round 0 is query-only — write/run tools were not dispatched. "
        + build_phase1_implementation_nudge(prompt)
    )


def mandatory_two_phase_active() -> bool:
    try:
        from .config import AGENTIC_MANDATORY_TWO_PHASE
    except Exception:
        from config import AGENTIC_MANDATORY_TWO_PHASE
    return bool(AGENTIC_MANDATORY_TWO_PHASE)
...
def should_force_implementation_batch(
    *,
    ...
    mandatory_two_phase: bool | None = None,
) -> bool:
    ...
    if not is_actionable_notebook_request(prompt):
        return False
    if mandatory_two_phase is None:
        mandatory_two_phase = mandatory_two_phase_active()
    if mandatory_two_phase and round_idx is not None and int(round_idx) >= 1:
        if not parsed_tools or is_query_only_tool_batch(parsed_tools):
            return True
    if not parsed_tools:
        return False
