...
try:
    from .agentic_action_guard import PHASE0_QUERY_TOOLS
except Exception:
    from agentic_action_guard import PHASE0_QUERY_TOOLS
...
def filter_tools_for_phase(
    ...
    *,
    ...
    mandatory_two_phase: bool = False,
) -> tuple[list[dict], dict[str, Any]]:
    """
    Phase 0: query-only (get_cell, list_cells, snapshot_status); strip writes or auto-inject reads.
    Phase 1: write/run only; strip query tools (especially list_cells).
    """
    meta: dict[str, Any] = {
        "writes_stripped": False,
        "queries_stripped": False,
        "auto_injected": False,
    }
    if not mandatory_two_phase or not tool_calls:
        return list(tool_calls), meta

    names = [_tool_name_from_raw(tc) for tc in tool_calls]

    if int(phase) == 0:
        writes_present = any(
            n in BROWSER_WRITE_TOOLS or n in RUN_TOOLS for n in names
        )
        allowed = [tc for tc in tool_calls if _tool_name_from_raw(tc) in PHASE0_QUERY_TOOLS]
        if writes_present:
            meta["writes_stripped"] = True
        if allowed:
            return allowed, meta
        try:
            from .agentic_action_guard import build_auto_injected_query_tool_call
        except Exception:
            from agentic_action_guard import build_auto_injected_query_tool_call
        injected = build_auto_injected_query_tool_call(
            prompt,
            url=url,
            tab_id=tab_id,
        )
        meta["auto_injected"] = True
        if writes_present or not tool_calls:
            return [injected], meta
        return allowed or [injected], meta

    if int(phase) >= 1:
        filtered = [
            tc for tc in tool_calls
            if _tool_name_from_raw(tc) not in READ_TOOLS
        ]
        if len(filtered) < len(tool_calls):
            meta["queries_stripped"] = True
        return filtered, meta

    return list(tool_calls), meta


def _is_mandatory_phase0_query_round(trace_round: int | None) -> bool:
    """True when mandatory two-phase is on and this is the query-only API round."""
    if trace_round is None:
        return False
    try:
        from .config import AGENTIC_MANDATORY_TWO_PHASE
    except Exception:
        from config import AGENTIC_MANDATORY_TWO_PHASE
    return bool(AGENTIC_MANDATORY_TWO_PHASE) and int(trace_round) == 0


def _batch_tool_call_args(call: ParsedToolCall) -> dict[str, Any]:
    """Tool args for registry.call; agentic batch writes default to fire-and-forget."""
    args = dict(call.args)
    try:
        from .config import AGENTIC_FIRE_AND_FORGET
    except Exception:
        from config import AGENTIC_FIRE_AND_FORGET
    if AGENTIC_FIRE_AND_FORGET and call.name in BROWSER_WRITE_TOOLS | RUN_TOOLS:
        args.setdefault("fire_and_forget", True)
        args.setdefault("wait_for_result", False)
    return args


def _split_source_into_parts(source: str, n: int) -> list[str]:
    ...
    for part_i in range(n):
        if part_i == n - 1:
            ...
            parts.append("\n".join(lines[idx: idx + per]))
            idx += per
    return parts


def enrich_split_cell_batch(
    ...
    *,
    ...
    """Add insert+edit chain when split request only edits/shrinks the source cell."""
    try:
        from .agentic_action_guard import split_missing_inserts
        from .agentic_tool_chain import (
            extract_split_cell_count_from_prompt,
            extract_target_cell_index,
        )
    except Exception:
        from agentic_action_guard import split_missing_inserts
    ...
    if not split_missing_inserts(user_prompt, calls):
        return calls

    target = extract_target_cell_index(user_prompt)
    if target is None:
        return calls
    n = extract_split_cell_count_from_prompt(user_prompt) or 3
    source = ""
    if registry is not None:
        try:
            result = registry.call(
                "notebook_get_cell",
                {"url": url, "cell_index": int(target)},
            )
            if isinstance(result, dict):
                source = str(result.get("input") or result.get("content") or "")
        except Exception:
            pass
    existing_edit = next(
        (c for c in calls if c.name == "edit_cell_by_index"),
        None,
    )
    if existing_edit and str(existing_edit.args.get("content") or "").strip():
        parts = [str(existing_edit.args.get("content"))]
        while len(parts) < n:
            parts.append("")
    elif source:
        parts = _split_source_into_parts(source, n)
    else:
        parts = [""] * n

    preserved = [
        c for c in calls
        if c.name not in {"insert_cell", "edit_cell_by_index"}
    ]
    out = list(preserved)
    anchor = int(target)
    for i in range(max(0, n - 1)):
        ...
        out.append(
            ParsedToolCall(
                id=f"host_split_ins_{i}",
                name="insert_cell",
                args=ins_args,
            )
        )
    for i, content in enumerate(parts[:n]):
...
def _cell_index_from_raw(tc: dict) -> int | None:
    fn = (tc.get("function") or {}) if isinstance(tc, dict) else {}
    raw_args = fn.get("arguments") or "{}"
    try:
        args = json.loads(raw_args) if isinstance(raw_args, str) else dict(raw_args or {})
...
def _insert_anchor_from_raw(tc: dict) -> int | None:
    fn = (tc.get("function") or {}) if isinstance(tc, dict) else {}
    raw_args = fn.get("arguments") or "{}"
    try:
        args = json.loads(raw_args) if isinstance(raw_args, str) else dict(raw_args or {})
        ...
        args = {}
    for key in ("index", "cell_index", "below_index", "after_index"):
        try:
            return int(args.get(key))
        except (TypeError, ValueError):
            continue
...
_DELETE_REORDER_TOOLS = frozenset({"delete_by_index"})
_INSERT_REORDER_TOOLS = frozenset({"insert_cell", "notebook_insert_cell"})


def _reorder_tool_group_descending(
    tool_calls: list[dict],
    tool_names: frozenset[str],
    index_fn,
) -> tuple[list[dict], bool]:
    """Stable-sort one tool group by index descending; other calls keep relative order."""
    if not tool_calls:
        return tool_calls, False
    group = [(i, tc) for i, tc in enumerate(tool_calls) if _tool_name_from_raw(tc) in tool_names]
    if len(group) <= 1:
        return tool_calls, False
    sorted_group = [
        tc for _, tc in sorted(
            group,
            key=lambda pair: (
                index_fn(pair[1]) if index_fn(pair[1]) is not None else -1,
                -pair[0],
            ),
            reverse=True,
        )
    ]
    it = iter(sorted_group)
    out: list[dict] = []
    for tc in tool_calls:
        if _tool_name_from_raw(tc) in tool_names:
            out.append(next(it))
        else:
            out.append(tc)
    return out, out != tool_calls


def reorder_deletes_descending(
    tool_calls: list[dict],
) -> tuple[list[dict], bool]:
    """Sort delete_by_index calls by cell_index highest-first before queue dispatch."""
    return _reorder_tool_group_descending(
        tool_calls, _DELETE_REORDER_TOOLS, _cell_index_from_raw
    )


def reorder_inserts_descending(
    tool_calls: list[dict],
) -> tuple[list[dict], bool]:
    """Sort insert_cell calls by anchor index highest-first before queue dispatch."""
    return _reorder_tool_group_descending(
        tool_calls, _INSERT_REORDER_TOOLS, _insert_anchor_from_raw
    )
...
def reorder_delete_before_insert(
    ...
    if not tool_calls:
        return tool_calls, False
    tool_calls, del_changed = reorder_deletes_descending(tool_calls)
    tool_calls, ins_changed = reorder_inserts_descending(tool_calls)
    struct_changed = del_changed or ins_changed
...
def _parsed_index_from_args(call: ParsedToolCall, keys: tuple[str, ...]) -> int | None:
    for key in keys:
        try:
            return int(call.args.get(key))
        except (TypeError, ValueError):
            continue
    return None


def _reorder_parsed_group_descending(
    calls: list[ParsedToolCall],
    tool_names: frozenset[str],
    keys: tuple[str, ...],
    ...
    if not calls:
        return calls, False
    group = [(i, c) for i, c in enumerate(calls) if c.name in tool_names]
    if len(group) <= 1:
        return calls, False
    sorted_group = [
        c for _, c in sorted(
            group,
            key=lambda pair: (
                _parsed_index_from_args(pair[1], keys)
                if _parsed_index_from_args(pair[1], keys) is not None
                else -1,
                -pair[0],
            ),
            reverse=True,
    ...
    for call in calls:
        if call.name in tool_names:
            ...
            out.append(call)
    return out, out != calls


def reorder_parsed_deletes_descending(
    calls: list[ParsedToolCall],
) -> tuple[list[ParsedToolCall], bool]:
    return _reorder_parsed_group_descending(
        calls, _DELETE_REORDER_TOOLS, ("cell_index", "index")
    )


def reorder_parsed_inserts_descending(
    calls: list[ParsedToolCall],
) -> tuple[list[ParsedToolCall], bool]:
    return _reorder_parsed_group_descending(
        calls, _INSERT_REORDER_TOOLS, ("index", "cell_index", "below_index", "after_index")
    )


def reorder_parsed_delete_before_insert(
    calls: list[ParsedToolCall],
) -> tuple[list[ParsedToolCall], bool]:
    if not calls:
        return calls, False
    calls, del_changed = reorder_parsed_deletes_descending(calls)
    calls, ins_changed = reorder_parsed_inserts_descending(calls)
    struct_changed = del_changed or ins_changed
    before = [c.name for c in calls]
    if "delete_by_index" not in before or "insert_cell" not in before:
        return calls, struct_changed
    if before.index("delete_by_index") < before.index("insert_cell"):
        return calls, struct_changed
    deletes = [c for c in calls if c.name == "delete_by_index"]
    inserts = [c for c in calls if c.name == "insert_cell"]
    other = [c for c in calls if c.name not in {"delete_by_index", "insert_cell"}]
    reordered = deletes + inserts + other
...
def reorder_parsed_runs_last(
    ...
    if not calls:
        return calls, False
    calls, struct_changed = reorder_parsed_delete_before_insert(calls)
    before = [c.name for c in calls]
...
def enrich_batch_from_prompt(
    ...
    *,
    ...
    if prompt_requests_delete(user_prompt) and "delete_by_index" not in names:
        original_indices = extract_delete_cell_indices(user_prompt)
        if original_indices:
            delete_indices = original_indices
            for i, delete_ci in enumerate(delete_indices):
                del_args: dict[str, Any] = {"cell_index": int(delete_ci), "url": url}
                if isinstance(tab_id, int) and tab_id > 0:
                    del_args["tab_id"] = tab_id
                out.append(
                    ParsedToolCall(
                        id=f"host_enrich_delete_{i}",
    ...
    if prompt_requests_insert(user_prompt):
        original_anchors = extract_insert_cell_indices(user_prompt)
        existing_inserts = [c for c in out if c.name == "insert_cell"]
        batch_anchors: list[int] = []
        for ins in existing_inserts:
            try:
                batch_anchors.append(int(ins.args.get("index") or ins.args.get("cell_index")))
            except (TypeError, ValueError):
                pass
        multi_fix = bool(
            original_anchors
            and len(original_anchors) > 1
            and (
                not existing_inserts
                or len(existing_inserts) != len(original_anchors)
                or len(set(batch_anchors)) < len(set(original_anchors))
                or set(batch_anchors) != set(original_anchors)
            )
        )
        if multi_fix:
            out = [c for c in out if c.name != "insert_cell"]
            for i, anchor in enumerate(original_anchors):
                ...
                out.append(
                    ParsedToolCall(
                        id=f"host_enrich_insert_{i}",
                        name="insert_cell",
                        args=ins_args,
                    )
                )
            names = {c.name for c in out}
        elif "insert_cell" not in names and original_anchors:
            anchor = original_anchors[0]
            ins_args = {
                "index": int(anchor),
                "direction": "below",
                "url": url,
...
def normalize_sequential_insert_anchors(calls: list[ParsedToolCall]) -> list[ParsedToolCall]:
    ...
    distinct_anchors: set[int] = set()
    for ins in inserts:
        try:
            distinct_anchors.add(int(ins.args.get("index") or ins.args.get("cell_index")))
        except (TypeError, ValueError):
            continue
    if len(distinct_anchors) > 1:
        sorted_anchors = sorted(distinct_anchors)
        is_sequential_chain = (
            len(sorted_anchors) == len(inserts)
            and sorted_anchors
            == list(range(sorted_anchors[0], sorted_anchors[0] + len(inserts)))
        )
        if not is_sequential_chain:
            return calls
...
def execute_agentic_batch(
    ...
    *,
    ...
    _phase0_query = _is_mandatory_phase0_query_round(trace_round)
    if _phase0_query:
        force_implementation = False
        parsed = [c for c in parsed if c.name in PHASE0_QUERY_TOOLS]
    if force_implementation and not _phase0_query:
        try:
    ...
    if not _phase0_query:
        ...
        parsed = enrich_split_cell_batch(
            parsed,
            user_prompt=user_prompt,
            url=url,
            tab_id=tab_id,
        ...
        parsed, _reordered = reorder_parsed_runs_last(parsed)
    try:
    ...
    for call_idx, call in enumerate(parsed):
        ...
        try:
            ...
            if not allowed:
                ...
                result = registry.call(call.name, _batch_tool_call_args(call))
        except Exception as exc:
            result = {"ok": False, "error": str(exc), "tool": call.name}
