...
The host entry point is \texttt{testing/host/host.py}. On startup it loads configuration from \texttt{config.py} (environment variables and \texttt{.env}), initialises data directories under \texttt{testing/host/data/}, and starts the native messaging I/O loop via the dispatcher module. Incoming messages are classified by type and routed to specialised modules: notebook ingestion (\texttt{notebook\_data\_handler.py}), streaming chat (\texttt{streaming.py}), and tool execution (\texttt{tool\_registry.py} and \texttt{agentic\_batch\_executor.py}).
...
Chat history is keyed by a stable notebook identity resolved from the Kaggle kernel id when available ({\ttfamily\seqsplit{kaggle:kernel:<id>}}) or from a normalised URL fallback (\texttt{notebook\_identity.py}). This key aligns snapshot filenames, SQLite session storage, and tool \texttt{url} arguments.
...
\begin{itemize}
  \item \textbf{Local read tools} execute entirely on the host against persisted JSON snapshots. Examples include \texttt{notebook\_list\_cells}, \texttt{notebook\_get\_cell}, and \texttt{notebook\_snapshot\_status}.
...
The batch executor (\texttt{agentic\_batch\_executor.py}) integrates with \texttt{strict\_execution\_engine.py} to build a deterministic queue. Bulk \texttt{delete\_by\_index} and \texttt{insert\_cell} operations are sorted in \emph{descending} index order on the host before browser dispatch, mitigating index-shift errors when multiple positional edits occur in one turn. \texttt{run\_cell} actions are ordered after structural mutations. Tool dispatches are logged through \texttt{tool\_call\_terminal.py} for development-time auditing.

Scrape-based observers include \texttt{cell\_execution\_observer.py} and \texttt{cell\_structure\_observer.py}. They update operational state asynchronously when cells run or the notebook layout changes, supplying feedback for monitors without a closed-loop replanning cycle in the current implementation.
